	...
	private_tpm_tss_tss2_session_t *this)
{
	...
	DBG2(DBG_PTS, LABEL "plaintext: %B", &data);
...
static bool rsa_salt(TPM2B_PUBLIC *public, TPMI_ALG_HASH alg, size_t len,
					 chunk_t *secret, TPM2B_ENCRYPTED_SECRET *encryptedSalt)
{
	...
	success = nonce_gen->allocate_nonce(nonce_gen, len, secret);
	nonce_gen->destroy(nonce_gen);
	if (!success)
	{
	...
	success = pubkey->encrypt(pubkey, encryption_scheme, &label, *secret,
								 &encrypted_salt);
...
static bool ecc_salt(TPM2B_PUBLIC *public, chunk_t *secret,
					 TPM2B_ENCRYPTED_SECRET *encryptedSalt)
{
	DBG1(DBG_PTS, LABEL "ECC salt not implmemented yet");
	return FALSE;
}
							...
							TPM2B_PUBLIC *public, TSS2_SYS_CONTEXT  *sys_context)
{
	...
	{
		case TPM2_ALG_RSA:
			DBG2(DBG_PTS, LABEL "EK RSA handle: 0x%08x", ek_handle);
			if (!rsa_salt(public, this->hash_alg, hash_len, &secret,
						  &encryptedSalt))
			{
				goto error;
			}
			break;
		case TPM2_ALG_ECC:
			DBG2(DBG_PTS, LABEL "EK ECC handle: 0x%08x", ek_handle);
			if (!ecc_salt(public, &secret, &encryptedSalt))
			{
				goto error;
			}
			break;
